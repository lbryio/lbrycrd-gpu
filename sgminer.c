...
static void gen_stratum_work(struct pool *pool, struct work *work)
{
  ...
  if (pool->algorithm.type == ALGO_NEOSCRYPT) {
    ...
    * into work. */
    ...
    /*if(pool->swork.has_trie)
    {
		// Copy ntime and nbits to the proper locations
		memcpy(work->data + pool->merkle_offset + 32, work->data + pool->merkle_offset + 64, 8);
		memcpy(work->data + pool->merkle_offset + 32, pool->swork.trie, 32);
	}*/
